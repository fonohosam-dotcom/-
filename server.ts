...
app.use(helmet({
  contentSecurityPolicy: false, // Vite requires unsafe-inline for dev, or config can be complex. Disabling CSP for simplicity in sandbox.
...
const AES_KEY_HEX = process.env.AES_SECRET_KEY || "d3b07384d113edec49eaa6238ad5ff0022f4c028b3e89cd3000b1a03efcb773d";
...
import { initialUsers, initialFamilies, initialCases, initialProjects, initialLedger, initialFunds } from "./src/server/seedData";
...
function saveState() {
  try {
    ...
    if (db) {
       ...
       db.collection("system").doc("state").set(state).catch(e => { db = null; });
    }
  } catch (e) {
